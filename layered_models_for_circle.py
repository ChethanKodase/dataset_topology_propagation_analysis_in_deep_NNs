...
class ConvoAE(nn.Module):
    ...
    def forward(self, x):
        encoded = self.encoder_l4(self.encoder_l3(self.encoder_l2(self.encoder_l1(x))))
        decoded = self.decoder_l1(self.decoder_l2(self.decoder_l3(self.decoder_l4(encoded))))
        return decoded
...
class VAE_mlp_circle_new(nn.Module):
    ...
    def representation(self, x):
        ...
        return self.bottleneck(self.encoder_l3(self.encoder_l2(self.encoder_l1(x))))[0]

    def forward(self, x):
        #h = self.encoder(x)
        h = self.encoder_l3(self.encoder_l2(self.encoder_l1(x)))
        z, mu, logvar = self.bottleneck(h.to(device))
        z = self.fc3(z)
        #print('z.shape', z.shape)
        #return self.decoder(z), mu, logvar
        return self.decoder_l1(self.decoder_l2(self.decoder_l3(z))), mu, logvar
